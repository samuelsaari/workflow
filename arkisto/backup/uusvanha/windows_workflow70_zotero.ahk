...
;ZOTERO
<!e:: 
if (WinActive("ahk_class MozillaWindowClass") && Winactive(ahk_exe zotero.exe) && WinExist("Quick Format Citation"))
	{
	WinActivate, Quick Format Citation
	Return
	}
...
else if (WinActive("ahk_class MozillaWindowClass") && Winactive(ahk_exe zotero.exe))
{
WinGet,WinState,MinMax,A
If WinState = -1
   WinMaximize
...
else if (WinExist("ahk_class MozillaWindowClass") && WinExist(ahk_exe zotero.exe))
	{
    	WinActivate, Zotero
	Winset,top,,zotero
	Return
